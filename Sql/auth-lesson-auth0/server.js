...
app.get('/auth/callback',
  passport.authenticate('auth0', {successRedirect: '/'}), function(req, res) { // or isunauthenticated it returns false instead an return res.send(404)
    res.status(200).send(req.user);
}) // when they are successful reroute to here '/' front page of website

app.get('/auth/me', function(req, res) {
  if (!req.user) return res.sendStatus(404);
  //THIS IS WHATEVER VALUE WE GOT FROM userC variable above.
  res.status(200).send(req.user);
}) // asks the server about the information about the user
